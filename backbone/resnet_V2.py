...
def bottleneck_2d(
    ...
    def f(x):
        y = keras.layers.Conv2D(filters, (1, 1), strides=stride, use_bias=False,
                                kernel_regularizer=ws_reg if ws else None,
        ...
        if block == 0:
            ...
            # shortcut = keras.layers.BatchNormalization(axis=axis, epsilon=1e-5, freeze=freeze_bn,
...
class ResNet2D(tensorflow.keras.Model):
    ...
    def __init__(
        ...
        x = keras.layers.Conv2D(
            64, (7, 7), strides=(2, 2), use_bias=False,
            kernel_regularizer=ws_reg if ws else None,
